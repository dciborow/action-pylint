...
if [[ "$(which pylint)" == "" ]]; then
  echo "[action-flake8] Installing pylint package..."
...
exit_val="0"
pylint --rc-file .pylintrc . 2>&1 | # Removes ansi codes see https://github.com/reviewdog/errorformat/issues/51
  /tmp/reviewdog -f=pylint \
